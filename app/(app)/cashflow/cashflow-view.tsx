    ...
    <section className="panel overflow-hidden bg-[#f8fbff] p-0">
      ...
      <div className="p-5">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          ...
          <button
            className="rounded-xl bg-accent px-5 py-3 text-sm font-semibold text-white shadow-[0_8px_18px_rgba(23,152,219,0.28)]"
            type="button"
                  ...
                  {data.transactions.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-3 py-8 text-center text-muted">
                        No transactions yet. Click &quot;+ Add New Expense&quot; to get started.
                      </td>
                    </tr>
                  ) : (
            ...
            <article className="rounded-2xl border border-line bg-white p-4 shadow-sm">
              ...
              <ul className="space-y-2 text-sm">
                <li className="flex items-center justify-between">
                  <span>Debt</span>
                  <span className="font-semibold">{mix.debt}%</span>
                </li>
                <li className="flex items-center justify-between">
                  <span>Personal</span>
                  <span className="font-semibold">{mix.personal}%</span>
                </li>
                <li className="flex items-center justify-between">
                  <span>Savings</span>
                  <span className="font-semibold">{mix.savings}%</span>
                </li>
              </ul>
              <p className="mt-3 border-t border-line pt-3 text-base font-semibold">
                Total Monthly Expenses {formatPHP(totalExpenses).replace("PHP", "₱")}
              </p>
            </article>
            ...
            <article className="rounded-2xl border border-[#102444] bg-gradient-to-br from-[#0f2140] to-[#12294e] p-5 text-white shadow-[0_12px_28px_rgba(16,33,64,0.35)]">
              ...
              <div className="mt-4">
                ...
                <div className="h-2 rounded-full bg-white/20">
                  <div className="h-2 w-[17%] rounded-full bg-white" />
                </div>
              </div>
            </article>

            <div className="grid grid-cols-2 gap-2">
              <button className="rounded-xl border border-line bg-white px-3 py-3 text-xs font-semibold" type="button">
                Download CSV
              </button>
              <button className="rounded-xl border border-line bg-white px-3 py-3 text-xs font-semibold" type="button">
                Budget Plan
        ...
        <TransactionForm open={showForm} onClose={() => setShowForm(false)} onSuccess={handleSuccess} />
      </div>
    </section>
  );
}
